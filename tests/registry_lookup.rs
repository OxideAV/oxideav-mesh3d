...
fn every_sibling_register_lands_encoders() {
    ...
    // each. USDZ ships no encoder.
    assert_eq!(
        formats,
        vec!["glb", "gltf", "mtl", "obj", "stl"],
...
fn encoder_for_extension_resolves_each_canonical_extension() {
    let reg = registry_with_all_formats();
    // USDZ has no encoder yet; everything else does.
    for ext in ["stl", "obj", "mtl", "gltf", "glb"] {
        assert!(
    ...
    assert!(
        reg.encoder_for_extension("usdz").is_none(),
        "USDZ ships a decoder only — no encoder factory should resolve"
    );
}
...
fn encoder_for_format_id_resolves_each_canonical_id() {
    let reg = registry_with_all_formats();
    for fmt in ["stl", "obj", "mtl", "gltf", "glb"] {
        assert!(
    ...
    assert!(
        reg.encoder_for_format("usdz").is_none(),
        "USDZ encoder absent",
    );
}
...
fn encoder_extensions_reverse_lookup() {
    ...
    assert!(
        reg.encoder_extensions("usdz").is_none(),
        "USDZ encoder doesn't exist; reverse lookup should miss"
    );
}
